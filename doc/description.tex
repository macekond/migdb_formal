...
\begin{align*}
 	\mathbf{state} = Consistent\; s \: | \: Inconsistent \; s,  \\	s \lhd State \vee s \lh Application \vee s \lhd Database
\end{align*}
...
\begin{align*}
  ...
  false
 \end{cases} \\ \\
	 c \in classes(application(s)), s \lhd State 
\end{align*}
...
\begin{align*}
	...
	\\ l \in Label, r \lhd Row \wedge r \in data(d), d \lhd Database
\end{align*}
\begin{align*}
...
\begin{align*}
 ...
 \end{cases}
	\\
	l_f \in Label, ps \lhd Label*, d \in Database
\end{align*}
...
\begin{align*}
   ...
   database(s) \bullet addTable(\rho(p,d), d) \\ \bullet \forall r \in row(owningClass(p)) :\\ insertRow(row(name(p), ["value", defaultValue(p)], \\ [name(p), value(primaryKey(r))])) 
    \end{gathered} } 
   \end{cases}\\
	s \lhd State, l \in Label, p \in Properties
\end{align*}
...
\begin{align*}
 ...
 \end{cases}
	\\
	l_c \in Label, ps \lhd Label*, s \lhd State
\end{align*}
...
\end{gathered}}
 \end{cases}\\
l_c, l_o, l_n \in Label,  s \lhd State
\end{align*}
...
\end{gathered}}
 \end{cases} \\
	l_c, l_o, l_n \in Label, a \lhd Applications, s \lhd Sate
\end{align*}
...
\begin{align*}
	...
	\Phi(CopyProperty(l_s, l_t, l_p, m_{l_s, l_t}), s) = \begin{cases}
		...
		\inference{\exists p \in properties(c), c \in classes(s) : \\ name(p) = l_p \wedge cardinality(p) > 1}{\begin{gathered}
			...
			\end{gathered}}
 \end{cases}\\
 l_s, l_t, l_p \in Label,  m_{l_s, l_t}) \in M, s \lhd State
\end{align*}
...
\end{gathered}}\\\\
 \end{cases}
	\\
	l_s, l_t \in Label, ps \in Label*,  m_{l_s, l_t} \in M, s \lhd State
\end{align*}
...
\end{gathered}}\\\\
 ...
 \end{cases}
	\\
	l_s, l_n \in Label, ps \lhd Label* s \lhd State 
\end{align*}
...
l_s, l_t \in Label, m_{l_s, l_t} \in M, s \lhd State, \\ \\
