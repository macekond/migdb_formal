...
\begin{align*}
	...
	state(s) = \begin{cases}
 		\inference{consistent(app(s)) \wedge \\ consistent(db(s)) \\ \wedge \rho(app(s)) = db(s)}{Consistent \; s}\\ \\
...
\begin{align*}
	...
	instantiated(c, s) = \begin{cases} \inference{\exists t \in tables(db(s)) : name(t) = name(c) \\ \wedge contains(t, db(s))}{true} \\ \\
  false
 \end{cases} \\ \\
	 c \in classes(app(s)), s \lhd Software 
\end{align*}

\subsubsection{Database consistency}
The database consistency is founded on the consistency of structure and consistency between structure and stored values. Each row in a database has to refer to a table in database schema and it has to have the structure corresponding to the one defined by a table, otherwise the database is inconsistent. The consistency of database is then:\begin{align*}
...
\begin{align*}
	...
	\Psi(s, addProperty(l, p)) = app(s) \circ addProp(l,p) \\
	\Phi(s, addProperty(l, p)) = \begin{cases}
    		\inference{cardinality(p) \leq 1}{db(s) \bullet addColumn(l, \rho(p,d), d)} \\ \\
   ...
   db(s) \bullet addTable(\rho(p,d), d) \\ \bullet \forall r \in row(owningClass(p)) :\\ insertRow(row(name(p), ["value", defaultValue(p)], \\ [name(p), value(primaryKey(r))])) 
    \end{gathered} } 
   \end{cases}\\
...
\Psi(s, addAsociation(l, a, f_m)) = app(s) \circ addAssoc(l, a) \\
\Phi(s, addAsociation(l, a, f_m)) = db(s) \bullet insertAssoc(l, a, f_m)
\end{align*}
...
\Psi(s, newClass(c)) = app(s) \circ addClass(c) \\
\Phi(s, newClass(c)) = db(s)  \bullet addTable(\rho(c,d), d) \\ \bullet addColumn(name(c), \{\forall r \in singleProperties(c) | \rho(r,d)\}, d) \\ \bullet addTable(name(c), \{\forall r \in collections(c) | \rho(r,d)\}, d)  
\end{align*}
...
\begin{align*}
	...
	\Phi RemoveClass(l, s) = db(s) \bullet dropTable(l) \\
...
\begin{align*}
	...
	\Phi RemoveProperty(l, l_p, s) = \begin{cases}
		 \inference{cardinality(propertyByName(l_p, app(s)) \leq 1}{db(s) \bullet dropColumn(l, l_p)}  \\\\
		 \inference{cardinality(propertyByName(l_p, app(s)) >1}{db(s) \bullet dropTable(l)}
 	\end{cases}\\
...
\begin{align*}
    ...
    \Psi RemoveAssociation(l, l_a, s) = app(s) \circ remAss(l, l_a) \\
    \Phi RemoveAssociation(l, l_a, s) = \begin{cases}
     	\inference{a = associationByName(l, l_a, app(s)) \\ \wedge startCardinality(a) \leq 1 \\}{db(s) \bullet dropForeignKey(l_a, reference(a))} \\\\
     	\inference{a = associationByName(l, l_a, app(s)) \\ \wedge startCardinality(a) > 1}{db(s) \bullet dropTable(l_a)} 
     \end{cases}\\
...
\Psi(RenameClass(l_o, l_n), s) = app(s) \circ renClass(l_o, l_n) \\
\Phi(RenameClass(l_o, l_n), s) = db(s) \bullet alterTable(l_o, l_n) \\ \\
\end{align*}

\paragraph{Rename Property}
\begin{align*}
	\mathbf{RenameProperty} : Label \times Label \times Label \times State \rightarrow State \\ \\
\Psi(RenameProperty(l_c, l_o, l_n, s)) = app(s) \circ renProp(l_c, l_o, l_n) \\
\Phi(RenameProperty(l_c, l_o, l_n, s)) = \begin{cases}
 	\inference{cardinality(propertyByName(l_c, l_o, app(s))) \leq 1}{db(s) \bullet alterColumn(l_c, l_o, l_n))} \\\\
 	\inference{cardinality(propertyByName(l_c, l_o, app(s))) > 1}{db(s) \bullet alterTable(l_o, l_n, db(s))}
 \end{cases}\\
...
\Psi(RenameAssocaiation(l_c, l_o, l_n), s) = app(s) \circ renAssoc(l_c, l_o, l_n)\\
\Phi(RenameAssocaiation(l_c, l_o, l_n), s) = \begin{cases}
	\inference{\exists a  \in associationByName(app(s)) : \\
  name(a) = l_c \\ \wedge startCardinality(a) > 1 }{db(s) \bullet alterTable(l_c, l_o, l_n)}
 \\ \\
  \inference{\exists a  \in associationByName(app(s)) : \\
  name(a) = l_c \\ \wedge startCardinality(a) \leq 1 }{\begin{gathered}
db(s) \\ \bullet alterForeignKey(endClass(a) ,l_c, l_o, l_n)
\end{gathered}}
 \end{cases}
\\\\
...
\begin{align*}
	...
	\Psi(CopyProperty(l_s, l_t, l_p, f_m), s) = \inference{\exists c \in classes(s) : name(c) = l_s \\ \wedge \exists p \in properties(c) : name(p) = l_p }{app(s) \circ addProp(l_t, p)} \\\\
	\Phi(CopyProperty(l_s, l_t, l_p, f_m), s) = \begin{cases}
		\inference{\exists p \in properties(c), c \in classes(s) : \\ name(p) = l_p \wedge cardinality(p) \leq 1 }{\begin{gathered}
			db(s) \bullet copyColumn(l_s, l_t, l_p) \\ \bullet copyValues(l_s, l_t, l_p, f_m) 
			\end{gathered}} \\ \\
		\inference{\exists p \in properties(c), c \in classes(s) : \\ name(p) = l_p \wedge cardinality(p) > 1}{\begin{gathered}
			db(s) \bullet copyTable(l_s, l_t, l_p) \\ \bullet copyRows(l_s, l_t, l_p, f_m) 
			\end{gathered}}
 \end{cases}
\end{align*}
