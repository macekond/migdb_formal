...
\paragraph{Add association} Adding an association into a database is complicated because of already existing instances. When an association between classes is added the instances of both classes has to be connected on the database level via foreign key or connection table. The information about pairing has to be provided by the user in form of set of instances pairs, because the model does not differ between one-to-one and (one)many-to-many relationships, however both cases are distinguished in the model. The set of instances pairs is in this paper called $O_{l_1, l_2, s}$ in case it represents one to one (or many) association or $M_{l_1, l_2, s}$ in case it represents many to many relationship, where $l_1, l_2$ are names of classes which are paired together and $s$ is software which creates the context of instances. 
\begin{align*}
	O_{l_1, l_2} = <(r_1, r_2)*>,\\ 
	r_1 \in tableData(l_1, database(s)) \Rightarrow \exists p \in O_{l_1, l_2} \wedge first(p) = r_1 \\
	r_2 \in tableData(l_2, database(s)) \Rightarrow \exists p \in O_{l_1, l_2} \wedge second(p) = r_2\\
	\forall p_1, p_2 \in O_{l_1, l_2} : first(p_1) = first(p_2) \Rightarrow p_1 = p_2 \\
...
\Phi(s, addAsociation(l, a, f_m)) = \begin{cases}
	\inference{ \exists t_s, t_t \in tables(d) : name(t_s) = l\\ 		\wedge name(t_t) = reference(a) \\ \wedge \forall f \in foreignKeys(t_t) \cuo columns(t_t) : name(f) \neq name(a) \\ startCardinality(a) \leq 1}{\begin{gathered}
		database(s) \bullet addForeignKey(name(t_s), \rho(a,d), d) \\ \bullet \forall r \in f_m(l, reference(a)) : \\ insertRow(row(name(t_s), [name(a), \\ value(primaryKey(first(r)))]), d)
	\end{gathered}}
 \\ \\
	\inference{ \exists t_s, t_t \in tables(d) : name(t_s) = l \\ \wedge name(t_t) = reference(a) \\ \wedge startCardinality(a) > 1}{\begin{gathered} 
	database(s) \bullet addTable(\rho(a,d),d) \\ \bullet \forall r \in f_m(l, reference(a)) :\\ insertRow(row(name(owningClass(a)) + name(a), \\ [name(t_s), value(primaryKey(first(r)))]\\ [name(t_t), value(primaryKey(second(r)))]))
	\end{gathered}} 
\end{cases}\\
\end{align*}
...
\begin{align*}
	...
	\Phi(CopyProperty(l_s, l_t, l_p, f_m), s) = \begin{cases}
		\inference{\exists p \in properties(c), c \in classes(s) : \\ name(p) = l_p \wedge cardinality(p) \leq 1 }{\begin{gathered}
			database(s) \bullet copyColumn(l_s, l_t, l_p) \\ \bullet copyValues(l_s, l_t, l_p, l_p, f_m) 
			\end{gathered}} \\ \\
		\inference{\exists p \in properties(c), c \in classes(s) : \\ name(p) = l_p \wedge cardinality(p) > 1}{\begin{gathered}
			database(s) \bullet copyTable(l_s + l_p, l_t + l_p) \\ \bullet copyRows(l_s + l_p, l_t + l_p, f_m) 
			\end{gathered}}
 \end{cases}
\end{align*}
