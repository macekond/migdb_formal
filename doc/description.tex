...
\end{align*}
The $insertProp$ on database level can change stored data too, because a mandatory property can be added into a class with already existing instances, then a default value has to be added to all existing instances.
\begin{align*}
...
\begin{align*}
	...
	\Phi RemoveProperty(l, l_p, s) = \begin{cases}
		 \inference{cardinality(propertyByName(l_p, application(s)) \leq 1}{database(s) \bullet dropColumn(l, l_p)}  \\\\
		 \inference{cardinality(propertyByName(l_p, application(s)) >1}{database(s) \bullet dropTable(l)}
 	\end{cases}\\
...
\begin{align*}
    ...
    \Phi RemoveAssociation(l, l_a, s) = \begin{cases}
     	\inference{a = associationByName(l, l_a, application(s)) \\ \wedge startCardinality(a) \leq 1 \\}{database(s) \bullet dropForeignKey(l_a, reference(a))} \\\\
     	\inference{a = associationByName(l, l_a, application(s)) \\ \wedge startCardinality(a) > 1}{database(s) \bullet dropTable(l_a)} 
     \end{cases}\\
...
\Phi(RenameProperty(l_c, l_o, l_n, s)) = \begin{cases}
 	...
 	\inference{cardinality(propertyByName(l_c, l_o, application(s))) > 1}{database(s) \bullet alterTable(l_o, l_n, database(s))}
 \end{cases}\\
