...
$
AddAssociation(l, s, a) = \inference{\forall f \in associations(a) : name(f) \neq name(s) \\ \wedge \exists c \in classes(a) : name(c) = l \\ \wedge \exists k \in classes(a) : name(k) = reference(s) }{associations(a) \cup s}
$
...
$ \rho(c) = table(name(c), primaryKey("Id"), \rho(primitiveProperties(c), \varnothing $\\$ \cup \rho(associations(c)) \cup \rho(collectionProperties)$
\end{center}
...
$\rho(p, d) = \begin{cases}
 \inference{cardinality(p) = 0 \\ \wedge \rho(owningClass(p)) \in classes(d)}{columns(\rho(owningClass(p))) \cup column(name(p), \rho(type(p)), \varnothing)} \\ \\
 
 \inference{cardinality(p) = 1 \\ \wedge \rho(owningClass(p) \in classes(d)}{columns(\rho(owningClass(p))) \cup column(name(p), \rho(type(p)), NOTNULL)} \\ \\
 
 \inference{cardinality(p) > 2 \\ \wedge \rho(owningClass(p) \in classes(d)}{\begin{gathered} table(name(p), primaryKey("Id"), column("value", \rho(type(p))), \\ foreignKey(name(p), name(owningClass(p))) \end{gathered}}
 \end{cases}$
...
$
\rho(a,d) = \begin{cases}
 \inference{startCardinality(a) = 0 \\ \wedge \rho(owningClass(p)) \in classes(d)  \wedge \rho(reference(a)) \in tables(d)}{
 \begin{gathered}
 foreignKeys(\rho(reference(a))) \\ \cup foreignKey(name(a), owningClass(a),  \varnothing) 
 \end{gathered}
 }
...
foreignKeys(\rho(reference(a))) \\ \cup  foreignKey(name(a), \\ owningClass(a),  NOTNULL)
\end{gathered}
} \\ \\
