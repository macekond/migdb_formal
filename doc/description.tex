...
\centering
	...
	\label{fig:evolution}
\end{figure}
The evolution of a software is a transformation from one consistent state to another one. These states are called generations of the software. The evolution consists of change in application and in database as shown in the Figure \ref{fig:evolution}. We focus on situations when the evolution is initiated on the application level (as code refactoring), however the evolution has to be processed on the database level too. It means there is a set of application transformations called $AppOps$ and a set of database transformations called $DbOps$ and a mapping between these two sets. We call this mapping $\Phi$:
\begin{gather*}
\Phi : AppOps \rightarrow DbOps
\end{gather*}

The evolution of the software is defined as the $\oplus$ operator:
$$\oplus : State \times AppOps \rightarrow State $$
and similarly the $\oplus_{app}$ and the $\oplus_{db}$ operators for evolving application and database:
\begin{gather*}
\oplus_{app} : Application \times AppOps \rightarrow Application \\
 \oplus_{db} : Database \times DbOps \rightarrow Database
\end{gather*}
The definition of the evolving operator is inspired by the monads (as used e.g. in Haskell) so the state of the evolution can be passed from one evolution step to another, which helps us to concatenate evolution transformations.
\begin{gather*}
 u \oplus t = \begin{cases}
 	\inference{u = Inconsistent \; s}{u} \\ \\
 	\inference{u = Consistent \; s}{state(soft(application(u) \oplus_{app} t, database(u) \oplus_{db} \Phi(t))} 
\end{cases}\\ 
u \in States, t \in AppOps, s \in Softwares
\end{gather*}
\begin{gather*}
 a \oplus o = \begin{cases}
 	\inference{!\;consistent(a)}{a} \\ \\
 	\inference{consistent(a)}{a \oplus_{app} o} 
\end{cases}\\ 
a \in Applications, o \in AppOps
\end{gather*}
\begin{gather*}
 d \oplus o = \begin{cases}
 	\inference{!\;consistent(d)}{d} \\ \\
 	\inference{consistent(d)}{d \oplus_{app} o} 
\end{cases}\\ 
