...
& \mathbf{sw} : Applications \times Databases \times ORM \rightarrow  Software \\ \\
...
& 	s : State \vee s \lh Application \vee s : Database
\end{align*}
...
& 	f(x, y) = \begin{cases}
 		...
 		\inference{otherwise}{Inconsistent y}
 \end{cases}\\\\
&  y : Software \vee y : Application \vee y : Database
\end{align*}
...
&  l \in Label,a : Applications, cs = Class* 
\end{align*}
...
& 	l \in Label, props : Property*, assocs : Association*
\end{align*}
...
& props' \subset props,  \forall p \in props' : cardinality(p) \leq 1 \\ \\
&	\mathbf{collections} : Class \rightarrow Property* \\	
& collections(cl(l, props, assocs)) = props',  \\
& props' \subset props, \forall p \in props' : cardinality(p) > 1 \\ \\
...
& 	associated(l,a) = \begin{cases}
 		...
 		false
 	\end{cases} \\ \\
&	l \in Label, props = Property*, assocs = Association*,a : Applications
\end{align*}
...
&	l \in Label, t : AppType, val \in Values, n \in \mathbb{N^{*}}, m \in Boolean 
\end{align*}
...
& l \in Label, t : DbType, val \in Values,\\ 
& cons : Constraint*, c : Column,d : Database
\end{align*}
...
&	contains(t, d) = \begin{cases}
			\inference{\exists r \in data(d) : refTable(r) = name(t) }{true} \\ \\
		 	false
 		\end{cases} \\ \\
& l \in Label, ps = Pair*, t \in tables(d), d : Database
\end{align*}
\begin{align*}
...
&	instantiated(c, s) = \begin{cases} \inference{\exists t \in tables(database(s)) : name(t) = name(c) \\ \wedge contains(t, database(s))}{true} \\ \\
  false
 \end{cases} \\ \\
&	 c \in classes(application(s)), s : State 
\end{align*}
...
%	l_c, l_p \in Label, a : Application
%\end{align*}
%\begin{align*}
...
%	l_c, l_a \in Label, a : Application
%\end{align*}
...
&	l \in Label, p : Property, a : Applications
\end{align*}
...
&	l \in Labels, s \in Associations, a : Applications
\end{align*}
...
&	l_c, l_p \in Label, a : Applications 
\end{align*}
\begin{align*}
...
&	l_c, l_a \in Labels, a : Applications
\end{align*}
...
&	renProp(l_c, l_o, l_n, a) = \inference{\exists c \in classes(a) : name(c) = l_c \\ \wedge \exists p \in properties(c) : name(p) = l_o \\ \wedge \forall e \in properties(c) : name(e) \neq l_n
		...
		p.name = l_n 
	\end{gathered}}\\
&	l_c, l_o, l_n \in Label, a : Application
\end{align*}
\begin{align*}
...
\end{gathered}}
 \\
& l_c, l_o, l_n \in Label, d : Database
\end{align*}
...
&	alterTable(l_o, l_n, d) = \inference{\exists t \in tables(d) : name(d) = l_o \\ \wedge \forall e \in tables(d) : name(e) \neq l_n}{\begin{gathered}
	...
	\end{gathered}}\\
&	l_o, l_n \in Label, s \in States, a : Applications, d : Database
\end{align*}
...
&	copyValue(l_s, l_t, l_{sc}, l_{tc} o_{l_s, l_t}, d) = \begin{cases}
		...
		\end{gathered}}
 	\end{cases} \\
& 	l_s, l_t, l_{sc}, l_{tc} \in Label, d : Database
\end{align*}
...
&	copyRows(l_s, l_t, m_{l_s, l_t}, d) = \begin{cases}
  ...
  \end{cases} \\
&  l_s, l_t \in Label, d : Database, m_{l_s, l_t} \in M
\end{align*}
...
& l \in Label, r : Row \wedge r \in data(d), d : Database
\end{align*}
\begin{align*}
...
&	dropTableAndData(l, d) = \inference{\exists t \in tables(d) \\ \wedge ! \; associated(t, d)}{\begin{gathered}
	(\forall r \in tableData(t,d) : dropRow(r, d))\\ \bullet dropTable(t,d)
\end{gathered}}
	\\& l \in Label, d : Database
\end{align*}
\begin{align*}
...
&	DropColumnAndData(l_c, ps, d) = \begin{cases}
 	...
 	(\inference{|ps| = 1 \\ \wedge \exists t \in tables(d) \\ \wedge \exists c \in columns(t) : name(c) = l_c}{\begin{gathered}
	 	(\forall r, r \in tableData(d) \wedge reference(r) = l_c: \\d \bullet dropValue(ps, r)) \end{gathered}}) \\ \bullet dropColumn(l_c)  
 \end{cases}
	\\
&	l_c \in Label, ps : Label*, d \in Database
\end{align*}

\begin{align*}
&	\mathbf{DropKeyAndData} : Label \times Label* \times Database \rightarrow Database \\
&	DropKeyAndData(l_f, ps, d) = \begin{cases}
 	%\inference{ps = \varnothing}{d}\\
 	 %\inference{|ps| >1}{\begin{gathered} 	d \bullet DropColumnAndData(l_c, head(ps), d) \\ \bullet DropColumnAndData(l_c, tail(ps), d) \end{gathered}} \\ \\
 	(\inference{|ps| = 1 \\ \wedge \exists t \in tables(d) \\ \wedge \exists f \in foreignKeys(t) : name(f) = l_f}{\begin{gathered}
	 	(\forall r, r \in tableData(d) \wedge reference(r) = l_f: \\d \bullet dropValue(ps, r)) \end{gathered}}) \\ \bullet dropKey(l_f)  
 \end{cases}
	\\
&	l_f \in Label, ps : Label*, d \in Database
\end{align*}
...
&	\Phi(s, addProperty(l, p)) = \begin{cases}
   ...
   database(s) \bullet addTable(\rho(p,d), d) \\ \bullet \forall r \in row(owningClass(p)) :\\ insertRow(row(name(p), ["value", defaultValue(p)], \\ [name(p), value(primaryKey(r))])) 
    \end{gathered} } 
   \end{cases}\\
&	s : State, l \in Label, p \in Properties
\end{align*}
...
& \Phi(s, newClass(c)) = database(s)  \bullet addTable(\rho(c,d), d) \\ 
& \bullet addColumn(name(c), \{\forall r \in singleProperties(c) | \rho(r,d)\}, d) \\
& \bullet addTable(name(c), \{\forall r \in collections(c) | \rho(r,d)\}, d)  
\end{align*}
...
\end{gathered}}
 \end{cases} \\
&	l_c, l_o, l_n \in Label, a : Applications, s : Sate
\end{align*}
...
&	\Phi(CopyProperty(l_s, l_t, l_p, m_{l_s, l_t}), s) = \begin{cases}
		...
		\inference{\exists p \in properties(c), c \in classes(s) : \\ name(p) = l_p \wedge cardinality(p) > 1}{\begin{gathered}
			...
			\end{gathered}}
 \end{cases}\\
& l_s, l_t, l_p \in Label,  m_{l_s, l_t}) \in M, s : State
\end{align*}
...
\end{gathered}}\\\\
 \end{cases}
	\\
&	l_s, l_t \in Label, ps \in Label*,  m_{l_s, l_t} \in M, s : State
\end{align*}
...
&	state(s) \oplus Extract(l_s, ps, l_n) = \begin{cases}
	\inference{|ps| = 1}{\begin{gathered}
		s \oplus addClass(cl(l_n, \varnothing, \varnothing)) \\ \oplus MoveProperty(l_s, l_n, ps, \mathbf{X}) \end{gathered}}\\\\
	%\inference{|ps| > 1}{\begin{gathered} 	s \oplus Extract(l_s, head(ps), l_n) \\\oplus Extract(l_s, tail(ps), l_n) \end{gathered}}\\\\
	% \inference{|ps| = \varnothing}{state(s)}	 
 \end{cases}
	\\
&	l_s, l_n \in Label, ps : Label* s : State 
\end{align*}
...
& l_s, l_t \in Label, m_{l_s, l_t} \in M, s : State, \\ \\
