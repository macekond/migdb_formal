...
& \mathbf{sw} \in Applications \times Databases \times ORM \rightarrow  Software \\ \\
...
\begin{align*}
& 	\mathbf{consistent} \in State \rightarrow Boolean \\
...
\begin{align*}
&	\mathbf{state} \in Software \rightarrow State \\
&	state(s) = \begin{cases}
...
& state(s) \oplus t = \begin{cases}
 	\inference{! consistent(state(s))}{Inconsistent s} \\ \\
...
&  l \in Label,a \in Applications, cs = \{x | x \in Class\} 
\end{align*}
...
&	instantiated(c, s) = \begin{cases} \inference{\exists t \in tables(database(s)) : name(t) = name(c) \\ \wedge contains(t, database(s))}{true} \\ \\
  false
 \end{cases} \\ \\
&	 c \in classes(application(s)), s \in State 
\end{align*}
...
&	l \in Label, p \in Property, a \in Applications
\end{align*}
...
&	alterTable(l_o, l_n, d) = \inference{\exists t \in tables(d) : name(d) = l_o \\ \wedge \forall e \in tables(d) : name(e) \neq l_n}{\begin{gathered}
	...
	\end{gathered}}\\
&	l_o, l_n \in Label, s \in States, a \in Applications, d \in Database
\end{align*}
...
& \Phi(s, addAsociation(l, a, m_{l_1, l_2})) = \begin{cases}
...
\end{gathered}}
 \end{cases} \\
&	l_c, l_o, l_n \in Label, a \in Applications, s : Sate
\end{align*}
...
&	state(s) \oplus Extract(l_s, ps, l_n) = \begin{cases}
 ...
 \end{cases}
	\\
&	l_s, l_n \in Label, ps : Label* s \in State 
\end{align*}
...
& ps = \{x | x \in \forall c \in classes(applications)) : \forall p \in properries(c) : name(p))\}
\end{align*}
