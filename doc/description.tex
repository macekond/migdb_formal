...
\begin{gather*}
	copyColumn : Label \times Label \times Label \times Database \rightarrow Database \\
...
\begin{gather*}
insertRow : Row* \times Database \rightarrow Database \\
...
\begin{gather*}
appendValues : Label \times Pair* \times Database \rightarrow Daatabase \\
appendValues(l, p, d) = \begin{cases}
 	\inference{|p| > 1}{appendValues(l, head(p), d)\bullet appendValues(l, tail(p), d)} 
 \\ \\
 	\inference{|p| = 1 \wedge \exists r \in data(d), reference(r) = l}{pairs(r) \cup [name(p), defaultValue(p)]}
 \\ \\
 	 Inconsistent \; d
 \end{cases} 
 \\
...
\begin{gather*}
insertPair : Row \times Pair \times Database \rightarrow Database \\
insertPair(r_t, p, d) = \begin{cases}
 \inference{\forall e \in pairs(r_t) : reference(e) \neq reference(p) \\ \wedge \exists t \in tables(d) : name(t) = refernces(r_t), \\ \wedge \exists c \in columns(t) : reference(p) = name(c)}{ pairs(r_t) \cup p}
 \\\\
 Inconsistent \; d
 \end{cases}
\end{gather*}
...
\begin{gather*}
copyValue : Label \times Label \times Label \times (Instancece \times Instance) \times Database \times Database \\
...
\forall p \in f_m(t_s, t_t)  : \\ insertPair(second(p), [l_{tc},value(pairByRef(l_{sc}, first(p))),d)
\end{gathered}}
 \end{cases}
\end{gather*}
...
\\ \\
 ...
 \inference{ cardinality(p) > 1 \wedge !\;mandatory(p) \\ \wedge \exists t : t = table(name(owningClass(p)), d)}{\begin{gathered}table(name(p), primaryKey("Id"), \\ column("value", \rho_t(type(p)), \varnothing), \\ foreignKey(name(p), name(owningClass(p)), NOTNULL) \\
\end{gathered}}
\\ \\
...
\begin{gather*}
	CopyProperty : Label \times Label \times Label \times (Instance \times Instance) \times State \rightarrow State 
\\
\Psi(CopyProperty(l_s, l_t, l_p, f_m), s) = application(s) \circ copyProp(l_s,l_t, l_p) \\
\Phi(CopyProperty(l_s, l_t, l_p, f_m), s) = database(s) \bullet copyColumn(l_s, l_t, l_p)\\  \bullet copyValues(l_s, l_t, l_p, f_m) \\ 
\end{gather*}
